...
int checkVictory(gameState &game)
{
    ...
    for (int i = 0; i < 25; i++)
    {
        if (game.board[i % 5][i / 5] == -1)
            blueFound = true;
        else if (game.board[i % 5][i / 5] == 1)
            redFound = true;
...
{
    if (pos.x < 0 || pos.x >= 5)
        return -2; // Error X not in Range;
    if (pos.y < 0 || pos.y >= 5)
        return -3; // Error Y not in Range;
    ...
    for (i = -1; i <= 1; i++)
    {
        for (j = -1; j <= 1; j++)
        {
            ...
            else
            {
                ...
                if (colorAtPos == 0)
                    move = ((killOnly) ? 0 : 1);
                else if (colorAtPos == -color && getValueAtPosition(game, {pos.x + (2 * j), pos.y + (2 * i)}) == 0)
                    move = 2;
            }
            moves[j + 1][i + 1] = move;
        }
...
turnData playTurn(gameState &game, Coordinates pos, Coordinates move)
{
    ...
    if (possibleMoves[move.x][move.y] == 0)
        return {0};
